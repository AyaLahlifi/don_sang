...
class configure_admin_2fa extends FormBase
{
    ...
    public function render_totp_based_2fa_configure(array &$form, &$form_state, $authMethod, $response)
    {
        ...
        $customization_note = $App_Name === 'GOOGLE' ? '<div class="mo_2fa_highlight_background_note">Scan the below QR code and the account will be added with name <strong> ' . $googleAppName . ' </strong>. You can customize the account name from <a href="' . MoAuthUtilities::get_mo_tab_url('LOGIN') . '">2FA Policy for End Users</a> tab and under the <u>Customization</u> section.</div><br>' : '';
        ...
        $form['mo_scan_qr_code_google_authenticator']['actions_secret_key'] = array(
            '#markup' => t('<div class="qr_code_text">
                            <p><b>'.t('Can\'t scan the code? ').'</b>' .t('Add the code to google authenticator app manually:').'</p>
